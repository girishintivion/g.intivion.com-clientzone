...
?>
         ...
         <?php echo form_open('',array('class'=>'form-horizontal clearfix','id'=>'cashier5_direct_form','method'=>'post'));?>
          <?php 	            
            $token = md5(uniqid(rand(), TRUE));            
            if(isset ($_SESSION['form_token_cashier5_backoffice']))
            {
            	unset($_SESSION['form_token_cashier5_backoffice']);
            }            
			$_SESSION['form_token_cashier5_backoffice'] = $token;	
			?>
            <input type="hidden" name="my_token_cashier5_backoffice" value="<?php echo $token;?>">
    ...
    <h4 class="lt">loading</h4>
              ...
              <div class="col-xs-12 col-xxs spacetop1x text-center">
                <button type="submit" id="cashier5_direct_submit" class="btn-login transitions">SUBMIT</button>
              </div>
            </div>
          </form>
