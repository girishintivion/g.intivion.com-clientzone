<?php
class Cashier5 extends CI_Controller {
	...
	public function redirect()
	{
		...
		$redirect_url=base_url($this->uri->segment(1).'/cashier5/backoffice');
     ...
     public function backoffice()
     {
     	...
     	if($ip!='136.244.67.225' && $ip!='122.170.10.24' && $ip!='95.179.224.11' && $ip!='103.121.242.250'  && $ip!='43.242.228.6' && $ip=='157.33.235.26')
     	{
     		...
     		redirect(base_url($this->uri->segment(1).'/cashier5/access_denied'));
     	...
     	{
     		$data['gateway']='IpayTotal';
     		$this->load->view('templates/before-login-header', $data );
     		$this->load->view('traders_room/fund_account/realdeposits_direct_ipaytotal');
     		$this->load->view('templates/before-login-footer');
     	}
     	...
     	else
     	{
     		...
     		$token = $this->input->post('my_token_cashier5_backoffice');
     		
     		$session_token = $_SESSION['form_token_cashier5_backoffice'];
     		
     		unset($_SESSION['form_token_cashier5_backoffice']);
     		...
     		if( (empty($token)) || (empty($session_token)) || ($token != $session_token) )
     		{
     			...
     			redirect(base_url($this->uri->segment(1).'/cashier5/backoffice'));
     		...
     		$myfile = fopen(logger_url_txt."Real_deposits_ipaytotal.txt", "a") or die("Unable to open file!");
     		...
     		*/
     		...
     		$return_url=base_url($this->uri->segment(1).'/cashier5/redirect');
     		
     		$notification_url=base_url('en/cashier5/notify');
     		
     		$validation_url=base_url('en/cashier5/validate');
     		...
     		$request=array(
     				"gateway"=>REAL_DEPOSITS_GATEWAY_IPAYTOTAL,
     				"address"=>$address1,
	...
	{
		/*
		$myfile = fopen(logger_url_txt."Real_deposits_success_ipaytotal.txt", "a") or die("Unable to open file!");
	...
	//	$_SESSION['pop_mes'] = 'Your transaction is successful.';
        ...
        {
        	$myfile = fopen(logger_url_txt."Real_deposits_validate_ipaytotal.txt", "a") or die("Unable to open file!");
        ...
        {
        	$myfile = fopen(logger_url_txt."Real_deposits_notify_ipaytotal.txt", "a") or die("Unable to open file!");
        	   ...
        	   if($this->form_validation->run() === FALSE)
				...
				$zipCode= $this->input->post('zip');
					...
					*/
					...
					$return_url=base_url($this->uri->segment(1).'/cashier5/success');
